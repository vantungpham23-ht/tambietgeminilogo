...
function createRuntimeCanvas(width, height) {
    ...
    throw new Error('🥺 Môi trường này chưa có Canvas — trình duyệt lạ quá!');
}

function get2dContext(canvas) {
    const ctx = canvas.getContext('2d', { willReadFrequently: true });
    if (!ctx) {
        throw new Error('😅 Không tạo được ngữ cảnh 2D Canvas');
    }
    return ctx;
}
...
async function getVideoContext(file) {
    ...
    if (!videoTrack) {
        input.dispose();
        throw new Error('🤔 File này không có track video nào cả');
    }
...
export async function detectGeminiVideoWatermark(file, options = {}) {
    ...
    try {
        ...
        if (!frames.length) {
            throw new Error('😭 Không tách được frame nào từ video để kiểm tra');
        }
...
export async function removeGeminiVideoWatermark(file, options = {}) {
    ...
    if (!detection.isConfident && options.allowLowConfidence !== true) {
        throw new Error('🤨 Logo hơi mờ — mình tạm dừng export. Bạn bật "cho phép độ tin cậy thấp" rồi thử lại nha~');
    }
    ...
    if (!canEncodeAvc) {
        throw new Error('🤖 Trình duyệt chưa hỗ trợ WebCodecs H.264/AVC — bạn dùng Chrome hoặc Edge bản mới giúp mình nha~');
    }
    ...
    try {
        ...
        if (!target.buffer) {
            throw new Error('😢 Xuất video thất bại — kết quả trống trơn');
        }
