...
function getVideoAlphaMap(size, options = {}) {
    ...
    if (!alphaSource) {
        throw new Error('🥺 Thiếu alpha map Gemini 96px — không tạo được mẫu logo video');
    }
...
export function detectDiamondVideoWatermarkFromFrames({
    ...
    if (!Array.isArray(frames) || frames.length === 0) {
        throw new Error('😅 Không có frame nào để kiểm tra logo');
    }
    if (!candidates.length) {
        throw new Error(`🤔 Hiện chưa hỗ trợ logo ${width}x${height} nha — bạn thử size khác giúp mình`);
    }
