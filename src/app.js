...
const TEXT = {
    loading: '⏳ Đang nạp tài nguyên...',
    size: 'Kích thước',
    watermark: 'Logo phát hiện',
    position: 'Vị trí',
    status: 'Trạng thái',
    removed: '🧹 Đã xóa sạch logo!',
    skipped: '😅 Không thấy logo nào để xoá — ảnh giữ nguyên nha',
    visibleResidual: '🧽 Xử lý xong rồi nhưng có thể còn vết mờ',
    possibleContentDamage: '🎨 Cho ra kết quả tốt nhất rồi — bạn kiểm tra vùng logo giúp nha',
    mixedQualityWarning: '🎨 Kết quả tốt nhất có thể — vài chỗ có thể còn vết',
    unsupported: '🤨 Trình duyệt chưa hỗ trợ sao chép ảnh',
    copied: '✅ Đã sao chép!',
    copy: '📋 Sao chép kết quả',
    copyFailed: '😅 Sao chép thất bại',
    unsupportedFile: '🤔 Bạn chọn ảnh JPG/PNG/WebP hoặc video MP4/WebM/MOV nha~',
    fileTooLarge: '⚠️ Ảnh quá 20MB — trang này chưa xử lý. Video sẽ qua trang video.',
    skippedLargeImages: '🫣 Đã bỏ qua ảnh quá 20MB',
    handoffVideo: '🎬 Đang chuyển sang trang video...',
    progress: 'Tiến trình',
    pending: '⏳ Chờ xử lý',
...
async function routeVideoFile(file) {
    try {
        ...
        hideLoading();
        console.error(error);
        setStatusMessage(error.message || '🤔 Chuyển trang video bị lỗi — bạn mở trang video rồi chọn lại file giúp mình', 'warn');
    }
}
...
async function consumePendingImageHandoff() {
    ...
    try {
        ...
        setStatusMessage(error.message || '😅 Đọc ảnh tạm thất bại — bạn chọn lại file giúp mình nha', 'warn');
    }
}
