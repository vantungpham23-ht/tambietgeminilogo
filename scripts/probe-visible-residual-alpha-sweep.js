...
const DEFAULT_OUTPUT_PATH = path.resolve('.artifacts/visible-residual-crops/latest/alpha-sweep/model-investigation-alpha-sweep.json');
const ALPHA_GAINS = Object.freeze([
...
async function probeRecord({ record, sampleRoot, resolveAlphaMap }) {
    ...
    const alphaMap = resolveAlphaMap(record.config);
    if (!position || !alphaMap) {
        return {
            file: record.file,
    ...
    const trials = ALPHA_GAINS.map((alphaGain) => {
        ...
        return summarizeTrial(alphaGain, candidate, original, position, alphaMap);
    });
    const safeTrials = trials.filter((trial) => !trial.hardReject);
    const bestBySeverity = [...safeTrials].sort((left, right) => left.severity - right.severity)[0] ?? null;
    const bestByHalo = [...safeTrials].sort((left, right) => left.positiveHaloLum - right.positiveHaloLum)[0] ?? null;
...
async function main() {
    ...
    const report = {
        ...
        summary: {
            total: probes.length,
            directAlphaGainCouldClearVisible: probes.filter((probe) => probe.bestBySeverity?.visible === false).length,
            hardRejectBestCount: probes.filter((probe) => probe.bestBySeverity?.hardReject === true).length
        },
        probes
    };
